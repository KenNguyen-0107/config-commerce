...
const BasicRow: React.FC<BasicRowProps> = (props) => {
	...
	if (Zone == "Breadcrumb" || Zone == "MainNavigation" || !children) return null;

	if (Variant === "PDPContent") return <PDPContent data={Info}>{children}</PDPContent>;
	...
	if (Variant === "Rating") {
		const rowBg =
			Info?.Url == "/"
				? "bg-muted-background gap-6 lg:gap-10 py-10"
				: "bg-white gap-10 pt-10 pb-30";
	...
	switch (Variant) {
		...
		case "NavSecondary":
			CustomClass.RowWrapper =
				"min-h-10 bg-secondary-background flex items-center justify-between flex-wrap lg:flex-nowrap px-2 lg:px-4 gap-0 lg:gap-8";
			break;
		case "CategoryBlock":
		...
		case "AppsOnlinePlanningTool":
			...
			break;
		case "BannerItem":
			CustomClass.RowWrapper = Info?.ProductNumber
				? "bg-white py-20 lg:my-0 px-4 lg:p-0"
				: "bg-muted-background py-20 lg:my-0 px-4 lg:p-0";
		...
		case "InstallationAdvice":
			CustomClass.RowWrapper =
				Info?.AttributeTypeContainer?.AttributeTypes?.length > 0
					? "bg-white"
					: "bg-muted-background";
			CustomClass.ColWrapper = "flex flex-col px-4 py-10 lg:py-20 gap-10";
			break;
		case "ImageList":
		...
		case "PdpServiceCard":
			...
			break;
		case "Videos":
			CustomClass.RowWrapper = "bg-muted-background py-10 px-4 lg:px-0";
			break;
		...
		case "CollapseText":
			...
			break;
		case "USP":
			CustomClass.RowWrapper = Info?.ProductNumber ? "bg-muted-background py-6" : "";
			break;
		...
		>
			{Array.isArray(children) ? (
				children.map((child: React.ReactNode, index: number) => (
					<div
						key={index}
						className={cn(RowGridMap[Columns?.[index] ?? 0], CustomClass.ColWrapper)}
					>
						{child}
					</div>
				))
