...
const BannerRelatedProducts: React.FC<{ data?: ProductProps[] }> = (
	props
) => {
	const { data } = props;
  ...
  useEffect(() => {
    ...
    const parentNode = BannerRelatedProductsRef.current?.closest("[data-component='BasicGrid']")
    const closestBasicBanner = parentNode?.querySelector("[data-component='BasicBanner']")
		...
		>
          ...
          navigation={data.length > 2}
				...
				>
							...
							<SwiperSlide key={index}>
								<div className="relative flex items-center justify-center h-[167px] lg:h-[334px] p-1 lg:p-2">
									<Image
										...
										alt={
											(product.ImageContainer?.Images?.[0]
												?.ImageAltText as string) ||
                  ...
                  <Link href={product.Url || ""} title={product.ProductTitle || ""} className="absolute top-0 left-0 right-0 bottom-0 z-[2]" />
								</div>
							</SwiperSlide>
						);
					})}
