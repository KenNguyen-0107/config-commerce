...
export async function GET(request: NextRequest, { params }: { params: { billToId: string, shipToId: string } }) {
  try {
    const url = `${API_BASE_URL}${API_ENDPOINTS.relatedShipTo(params.billToId, params.shipToId)}`
...
export async function PATCH(request: NextRequest, { params }: { params: { billToId: string, shipToId: string } }) {
  try {
