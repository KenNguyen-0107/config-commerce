...
export async function GET(request: NextRequest) {
  try {
    ...
    const queryString = new URL(request.url).search // This includes the '?'
    const cleanQueryString = queryString.startsWith("?") ? queryString.slice(1) : queryString

    let url = buildApiUrl(API_BASE_URL, API_ENDPOINTS.getShipTo, {
      ...DEFAULT_QUERY_PARAMS.getShipTo,
    })
    url = `${url}&${cleanQueryString}`
