const toJson = (type: string, jsonld: any) => {
	...
	const { logo, ...rest } = jsonLdObject;
	const restJson = JSON.stringify(rest, safeJsonLdReplacer);
	const logoJson = logo ? `"logo":${JSON.stringify(logo)}` : "";

	if (restJson === "{}" && logo) {
		return { __html: `{${logoJson}}` };
	}

	return {
		__html: logoJson ? restJson.replace(/}$/, `,${logoJson}}`) : restJson,
	};
};
...
const safeJsonLdReplacer: JsonReplacer = (() => {
	...
	return (_: string, value: JsonValue): JsonValue | undefined => {
		switch (typeof value) {
			...
			case "string":
				if (value === "logo") {
					return value; // Return as-is if value is 'logo'
				}
