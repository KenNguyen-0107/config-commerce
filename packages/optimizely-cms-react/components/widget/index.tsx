...
const RenderWidget = ({
  ...
  const childWidgets = allWidgets.filter((w) => w.ParentId === widget.Id && w.Zone && !["Breadcrumb", "MainNavigation"].includes(w.Zone));
  ...
  // Combine all props
  const componentProps = { ...widget, CategoryId: categoryId, Info: info };
    ...
    <Component {...componentProps}>
      {Object.entries(widgetsByZone).map(([zone, widgets]) => (
        <ComponentWrapper
          key={zone}
          style={
            componentProps.__typename === "FooterContainer" ?
            {
              gridArea: zone,
          ...
          {widgets.map((childWidget) => (
            <RenderWidget
              ...
              widget={{
                ...
                Info: info,
                Section:
                  (childWidget.__typename === "BasicRow" &&
                    childWidget.Variant) ||
                  childWidget.Section ||
