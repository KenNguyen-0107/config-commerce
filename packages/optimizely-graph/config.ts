...
import { OptiCmsSchema } from "./client/types";

export type {
	ContentGraphConfig,
	OptimizelyGraphConfigInternal,
	OptimizelyGraphConfig,
} from "./types";
export { OptiCmsSchema } from "./client/types";

export function readEnvironmentVariables(): Types.OptimizelyGraphConfig {
	const config: Types.OptimizelyGraphConfig = {
		secret: getOptional("OPTIMIZELY_GRAPH_SECRET", () =>
			getOptional("OPTIMIZELY_CONTENTGRAPH_SECRET")
		),
		app_key: getOptional("OPTIMIZELY_GRAPH_APP_KEY", () =>
		...
		opti_cms_schema: getSelection<OptiCmsSchema>(
			...
			[OptiCmsSchema.CMS12, OptiCmsSchema.CMS13, OptiCmsSchema.COMMERCE],
			OptiCmsSchema.COMMERCE
		),
	};
	...
	if (config.deploy_domain && config.deploy_domain != "" && config.deploy_domain.includes("://"))
		try {
			...
			config.deploy_domain = config.deploy_domain.substring(
				config.deploy_domain.indexOf("://") + 3
			);
		}
