...
export class ContentGraphClient extends GraphQLClient implements IOptiGraphClient {
	...
	protected set token(newValue: string | undefined) {
		...
		if (
			(newMode == AuthMode.Basic || newMode == AuthMode.HMAC) &&
			!validateConfig(this._config, false)
		) {
			throw new Error(
				"❌ [Optimizely Graph] Configuration is invalid for selected authentication mode"
			);
		}
	...
	public constructor(
		...
		// Validate inputs
		const optiConfig: OptimizelyGraphConfig = applyConfigDefaults(
		...
		if (
			!validateConfig(optiConfig, authMode == AuthMode.Public || authMode == AuthMode.Token, true)
		)
		...
		if (configFlags.stored) serviceUrl.searchParams.set("stored", "true");
		super(serviceUrl.href, {
			...
			responseMiddleware: (response) => {
				...
				if (isError(response)) {
					...
					console.log(
						`🔖 [Optimizely Graph] [Response Cost] ${JSON.stringify(
							(response.extensions as { cost?: number } | undefined)?.cost || 0
						)}`
					);
				}
			},
	...
	public updateAuthentication(tokenOrAuthmode?: string | AuthMode | undefined): ContentGraphClient {
		if (tokenOrAuthmode == AuthMode.Token)
			throw new Error(
				"❌ [Optimizely Graph] Provide the authentication token to switch to AuthMode.Token"
			);
	...
	public updateFlags(
		newFlags: Partial<IOptiGraphClientFlags>,
		...
		if (this.debug)
			console.log(
				`🔵 [Optimizely Graph] ${
					temporary ? "Temporary updating" : "Updating"
				} the feature configuration of the client`
			);
	...
	protected updateRequestConfig(): void {
		...
		serviceUrl.searchParams.set("stored", this._flags.stored ? "true" : "false");
		...
		if (this._flags.cache_uniq) serviceUrl.searchParams.set("unique", "true");
		if (this.debug)
