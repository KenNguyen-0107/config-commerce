...
export class OptimizelyCommerceClient implements OptimizelyCmsRoutingApi {
	async getRoutes(client: GraphQLClient, siteId?: string): Promise<Route[]> {
		client.updateFlags({ stored: false /*, cache: false*/ }, true);
		let page = await client
			.request<GetAllRoutes.Result, GetAllRoutes.Variables>(GetAllRoutes.query, { domain: siteId })
			.catch((e) => {
	...
	async getRouteByPath(
		...
		if (client.debug)
			console.log(
				`⚪ [RouteResolver] Resolving content info for ${path} on ${
					siteId ? siteId : "all domains"
				}`
			);
		...
		if ((resultSet.getRouteByPath?.items?.length ?? 0) > 1)
			throw new Error(
				"🔴 [RouteResolver] Ambiguous URL provided, did you omit the domain in a multi-site setup?"
			);

		if (client.debug)
			console.log(
				`⚪ [RouteResolver] Resolved content info for ${path} to: ${JSON.stringify(
					resultSet.getRouteByPath.items[0]
				)}`
			);
	...
	async getRouteById(
		...
		if (resultSet.Content?.total >= 1) {
			if (client.debug && resultSet.Content?.total > 1)
				console.warn(
					`Received multiple entries with this ID, returning the first one from: ${(
