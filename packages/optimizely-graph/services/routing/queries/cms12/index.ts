...
export class OptimizelyCms12Client implements OptimizelyCmsRoutingApi {
	async getRoutes(client: GraphQLClient, siteId?: string): Promise<Route[]> {
		...
		let page = await client.request<GetAllRoutes.Result, GetAllRoutes.Variables>(
			GetAllRoutes.query,
			{ siteId, typeFilter: "Page" }
		);
	...
	async getRouteByPath(
		...
		if (client.debug)
			console.log(
				`Resolving content info for ${path} on ${siteId ? "site " + siteId : "all sites"}`
			);
