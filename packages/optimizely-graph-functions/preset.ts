...
export const preset : Types.OutputPreset<PresetOptions> =
{
    ...
    prepareDocuments: async (outputFilePath: Readonly<string>, outputSpecificDocuments: ReadonlyArray<Types.OperationDocument>) => {
        ...
        console.log({outputSpecificDocuments})
        const optiDocs = outputSpecificDocuments.filter<string>((x => typeof(x) == 'string' && x.startsWith('opti-commerce:')) as (x: Types.OperationDocument) => x is string)
        const normalDocs = outputSpecificDocuments.filter(x => !(typeof(x) == 'string' && x.startsWith('opti-commerce:')))
